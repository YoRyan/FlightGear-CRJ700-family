...
Engine.Apu = func (n) {
        ...
        apu.running = 0;
        apu.running_node = props.globals.getNode("/engines/apu[" ~ n ~
                        "]/running", 1);
        ...
        apu.on_fire = 0;
        apu.on_fire_node = props.globals.getNode("/engines/apu[" ~ n ~
                        "]/on-fire", 1);
        ...
        apu.update = func () {
                ...
                if (apu.serviceable and size(Engine.valid_fuel_tanks) > 0 and
                                apu.controls.on) {
...
Engine.Jet = func (n) {
        ...
        jet.controls.fire_ext_node = props.globals.getNode("/controls/engines/engine[" ~
                        n ~ "]/fire-bottle-discharge", 1);
        ...
        jet.controls.starter_node = props.globals.getNode("/controls/engines/engine[" ~
                        n ~ "]/starter", 1);
        ...
        jet.controls.throttle_node = props.globals.getNode("/fcs/throttle-cmd-norm[" ~
                        n ~ "]", 1);
        jet.controls.throttle_node.setValue(jet.controls.throttle);

        jet.fdm_throttle = 0;
        jet.fdm_throttle_node = props.globals.getNode("/controls/engines/engine[" ~
                        n ~ "]/throttle-lever", 1);

        jet.fdm_reverser = 0;
        jet.fdm_reverser_node = props.globals.getNode("/controls/engines/engine[" ~
                        n ~ "]/reverser", 1);
        ...
        jet.update = func () {
                ...
                if (!jet.serviceable or jet.out_of_fuel or jet.controls.cutoff) {
                        ...
                        jet.fdm_throttle = jet.fdm_throttle_idle + (1 -
                                        jet.fdm_throttle_idle) * jet.controls.throttle;
                        jet.n1 = jet.fdm_n1;
                        jet.controls.starter = 0;
