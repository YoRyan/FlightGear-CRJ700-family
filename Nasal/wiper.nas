...
var Wiper = func (control, position, power_signal, power_avail) {
        ...
        var move = func () {
                ...
                if (wiper.control == 0) {
                        delta = time_delta * 2;
                } elsif (wiper.control == 2) {
                        # We do not need to handle control == 1.
                        delta = time_delta * 2;
                } elsif (wiper.control == 3) {
                        delta = time_delta * 4;
                }
                if (going_down) {
                        wiper.position = math.min(wiper.position + delta, 1);
                } else {
                        wiper.position = math.max(wiper.position - delta, 0);
                }
        };
